...
def build():
    ...
    marks = '\n  '.join([
        ...
        callout(2, sheet_pt(FX, 168, 140)),
        callout(3, sheet_pt(FX, -16, 250)),
        callout(4, sheet_pt(FX, 138, 366)),
        callout(5, sheet_pt(FX, 150, 486)),
        callout(6, sheet_pt(FX, 212, 430)),
        callout(7, sheet_pt(FX, -500, 556), dark=True),
        callout(8, sheet_pt(FX, 566, 478), dark=True),
        callout(9, sheet_pt(BX, 0, 668)),
        callout(10, sheet_pt(BX, 0, 216)),
        callout(11, sheet_pt(BX, 0, 22)),
    ])
    ...
    legend, y1 = block(FX, 128, 'CALLOUTS', [
        ...
        ('5', ['Princess seams front and back - these carry the fit']),
        ('6', ['Shaped side seam, waist suppressed 6.4 cm from the chest']),
        ('7', ['Long cuff with thumbhole']),
        ('8', ['TERRA wraps the left cuff, NEXUS the right']),
        ('9', ['Scooped centre back hem, 4.6 cm below the side hem']),
        ('10', ['Back yoke seam']),
        ('11', ['Woven main label, centre back neck']),
    ], w=222, keyw=8, lead=3.9, size=2.75)

    m = block(FX, y1 + 6, 'MEASUREMENTS - SIZE M, AS DRAWN', [
        ('Fit', ['slim athletic']),
        ('Centre back length', [f'{J.LEN_B / 10:.1f} cm']),
        ('Centre front length', [f'{J.LEN_F / 10:.1f} cm']),
        ('Chest, laid flat', [f'{J.CHEST * 2 / 10:.1f} cm']),
        ('Waist, laid flat', [f'{J.WAIST * 2 / 10:.1f} cm']),
        ('Hem, laid flat', [f'{J.HEM * 2 / 10:.1f} cm']),
    ...
    grade = [('XS', 44.0, 62.8), ('S', 46.0, 64.8), ('M', 48.0, 66.8),
             ('L', 50.0, 68.8), ('XL', 52.5, 70.8), ('XXL', 55.0, 72.8)]
    ...
    trims, y3 = block(COL2, y2 + 5, 'TRIMS', [
        ...
        ('Cuff', ['66 mm bonded, thumbhole,', 'no elastic - the cuff is shaped']),
        ('Labels', ['Woven main at CB neck,', 'care label at left side seam']),
    ], w=118, keyw=32)

    brand, _ = block(COL2, y3 + 5, 'BRANDING', [
        ('Left cuff', ['TERRA, heat transfer, white,', f'{J.CAP_MM:.1f} mm caps, wrapping']),
        ('Right cuff', ['NEXUS, same treatment']),
        ('Elsewhere', ['None. No shoulder mark,', 'no sewn patch - patches are', 'bags only in this package.']),
    ], w=118, keyw=32)
  ...
  {txt(20, 27.5, 'WIND SHELL - HOODED, RAGLAN, SLIM ATHLETIC FIT', 3.2, INK, 'start', '500', FONT, 1.5, 0.75)}
  ...
  {chips}
  {txt(COL2, 94, 'One garment, three grounds. The cuff wordmarks stay white on every shell.', 2.5, INK, 'start', '400', FONT, 0, 0.7)}
  {fabric}
  {trims}
  {brand}

  <line x1="20" y1="{H - 14}" x2="{W - 20}" y2="{H - 14}" stroke="{INK}" stroke-opacity="0.25" stroke-width="0.3"/>
  {txt(20, H - 8.5, 'Production artwork: Output Drafts/Jacket/jacket-flat-front.svg', 2.6, INK, 'start', '400', MONO, 0, 0.6)}
